...
defmodule Lens2.Lenses.CombineTest do
  ...
  test "both" do
    ...
    assert Deeply.to_list(list, lens) == [0, 0, 2, 3, 4, 6, 6]
  ...
  describe "context" do
    test "getting returns both context/checkpoint and leaf" do
      ...
      assert Deeply.one!(map_list, lens) == {map_0, :target_1}

      lens = Lens.indices([0,1]) |> Lens.context(into_map)
      assert Deeply.to_list(map_list, lens) == [{map_0, :target_1},
                                                {map_1, :target_5}]
    ...
    test "multiple contexts" do
      ...
      [gotten_0, gotten_1] = Deeply.to_list(map_list, lens)
